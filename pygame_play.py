...
class VisualSnake:
    ...
    def print_score(self, score):
        value = self.font.render(f"Score: {score}", True, self.color.black)
        self.screen.blit(value, [500 * self.scale, 10])
        
    def print_episode(self):
        if self.show_episode:
            value = self.font.render(f"Episode: {self.episode}", True, self.color.black)
    ...
    def draw_snake(self):
        for i in range(len(self.snake_coords) - 1, -1, -1):
            ...
            if i == len(self.snake_coords) - 1:
                # head square color
                pygame.draw.rect(self.screen, self.color.violet, [x, y, self.snake_size, self.snake_size])
            else:
                pygame.draw.rect(self.screen, self.color.pink, [x, y, self.snake_size, self.snake_size])
            
    def game_end_message(self):
        mesg = self.font.render("Game over!", True, self.color.green)
