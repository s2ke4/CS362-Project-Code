...
class LearnSnake:
    ...
    def get_state(self):
        ...
        state.append(int(self.food_row < head_r))
        state.append(int(self.food_row > head_r))
        state.append(int(self.food_column < head_c))
        state.append(int(self.food_column > head_c))
    ...
    def generate_food(self):
        food_column = int(round(random.randrange(0, self.screen_width - self.snake_size) / 10.0))
        food_row = int(round(random.randrange(0, self.screen_height - self.snake_size) / 10.0))
        if self.board[food_row][food_column] != 0:
            food_row, food_column = self.generate_food()
    ...
    def step(self, action="None"):
        ...
        # insert snake head coordinate
        self.snake_coords.append((self.r1, self.c1))
        
        # filling the board index with head       
        ...
        if self.c1 == self.food_column and self.r1 == self.food_row:
            self.food_row, self.food_column = self.generate_food()
            self.board[self.food_row][self.food_column] = 2
            self.snake_length += 1
